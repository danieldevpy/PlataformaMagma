...
[
  ...
  {"nome": "escalar_contato",
   "descricao": "Marca um número como escalado pro humano E garante que ele exista na base de leads (mesmo sem nome e sem curso). A MAG para de responder automaticamente até a equipe resolver no admin ou o prazo expirar.",
   "parametros": {"numero": "string, só dígitos com DDI", "motivo": "string, por que está escalando", "nome": "string, opcional", "curso_slug": "string, opcional"},
...
| `identificar_contato` | nucleo | `nucleo:identificar_contato` | `numero` | `{papel, nome, escalado}` — `papel` em `gestor`\|`instrutor` (via `Usuario.whatsapp`/`papel`, sem modelo novo) \| `lead` (via `Lead.whatsapp`) \| `desconhecido` (`nome: null`, nunca inventa dado); `Usuario` tem prioridade sobre `Lead` no mesmo número; `escalado` = existe `ContatoEscalado` **ativo** pra esse número, ou seja, não resolvido e não expirado (spec 025 — antes bastava o registro existir, e não havia caminho de volta) |
| `escalar_contato` | nucleo | `nucleo:escalar_contato` | `numero`, `motivo`, `nome` (opcional), `curso_slug` (opcional) | `{ok: true, expira_em, lead_id}` — cria/renova o `ContatoEscalado` **ativo** do número (reescalar não é erro; se o anterior já estava resolvido, abre um registro novo — `numero` deixou de ser `unique` na spec 025) **e garante o `Lead`** por `whatsapp`, mesmo sem nome e sem curso, com `status=em_atendimento` e `utm_source=whatsapp`. `curso_slug` passa pelo mesmo `resolver_curso()` tolerante da spec 023, então slug inventado pelo modelo ainda resolve. `expira_em` sai de `ConfiguracaoSite.handoff_expira_horas` (padrão **24h**; `0` = nunca expira → devolve `null`) — spec 025 |
...
| `info_institucional` | nucleo | `nucleo:info_institucional` | `{}` | `{endereco, whatsapp_principal, instagram, email, nota_google, total_alunos_formados}` — os 4 primeiros sempre vêm do `ConfiguracaoSite` singleton; `nota_google`/`total_alunos_formados` vêm `null` quando o respectivo toggle (`exibir_nota_google`/`exibir_total_formados`) está desligado (diferente do serializer público de `GET /api/site/config/`, que expõe os dois campos crus pro front decidir exibir — aqui quem "decide" é a fala do agente, então o toggle já filtra na origem) — spec 017 |
| `resumo_diario` | nucleo | `nucleo:resumo_diario` | `{}` | `{leads_24h, turmas_abertas: [{turma_codigo, curso, vagas_restantes}], avaliacoes_pendentes, postagens_hoje, uso_ia_mes: {execucoes_ok, execucoes_erro, tokens_entrada, tokens_saida}}` — `leads_24h` conta janela rolante de 24h (não dia corrido, diferente de `listar_leads`); `turmas_abertas` só turmas com `status=inscricoes`; `postagens_hoje` conta `agendada_para` = hoje excluindo `status=publicada`; `uso_ia_mes` é a mesma agregação de `GET /api/ia/uso/` (mês corrente). Chamada por um `HTTP Request` comum no workflow do Radar, não por um AI Agent — resumo factual, sem geração — spec 019 |
| `processar_nutridora` | leads | `leads:processar_nutridora` | `{}` | `{processados: [{numero, texto}], total}` — processa a régua T+1/T+3/T+7 dos leads elegíveis (exclui `whatsapp` vazio, números com `ContatoEscalado` **ativo** (spec 025 — antes qualquer registro excluía pra sempre, então quem foi escalado uma vez nunca mais era nutrido) e números com `Conversa` nos últimos `ConfiguracaoSite.nutridora_silencio_dias` (padrão **2**, `0` = não checar). **028-T20**: a exclusão por `utm_source=whatsapp` foi **removida** — usava a ORIGEM do lead como proxy da ATIVIDADE dele e, como o `registrar_lead` do SDR carimba esse valor fixo no workflow, nenhum lead nascido de conversa entrava na régua; para esses leads a régua agora conta a partir da **última conversa**, não da criação), marca `Lead.nutridora_ultimo_toque` e devolve o texto pronto (dado real: habilidades do curso no T+1, avaliação aprovada no T+3 — se não houver nenhuma ainda, não avança e tenta de novo no dia seguinte —, vagas restantes no T+7 se `exibir_vagas`). No máximo 1 toque por lead por execução, mesmo que o lead esteja atrasado em vários estágios. Chamada por `HTTP Request` comum (sem AI Agent) — spec 020 |
| `listar_gestores` | nucleo | `nucleo:listar_gestores` | `{}` | lista de `{nome, whatsapp}` — gestores ativos (`Usuario.papel=gestor`) com WhatsApp cadastrado, excluindo instrutores e quem não tem número. Usada pelos workflows proativos (Radar, handoff) pra mandar aviso pra todos os gestores em vez de um número fixo hardcoded no n8n — spec 019, adendo 2026-07-24 |
| `registrar_turnos` | conversas | `conversas:registrar_turnos` | `{numero*, texto_contato, texto_agente, agente, papel, nome, ferramentas: [{nome, params}], execucao_n8n, proativo}` | `{conversa_id, conversa_nova, turnos_gravados, desfecho}` — grava a troca na `Conversa` do número (mesma conversa se houve atividade nas últimas **6h**, senão abre uma nova) e deriva `desfecho` das ferramentas chamadas, sem LLM: `registrar_lead`→`lead_registrado`, `escalar_contato`→`handoff` (+`escalada=True`), `matricular_aluno`→`matricula`, `gerar_cobranca`→`cobranca`, mantendo sempre o ponto mais avançado do funil. Aceita só `texto_agente` (+`proativo: true`) pros toques da Nutridora, que não respondem nada. Chamada por `HTTP Request` comum **depois** do envio no WhatsApp, com `onError: continueRegularOutput` — registro nunca atrasa nem derruba o atendimento — spec 021 |
| `exportar_conversas` | conversas | `conversas:exportar_conversas` | `{dias, agente, desfecho, numero, limite}` | `{periodo_dias, total, devolvidas, conversas: [{id, numero, nome, papel, agente, escalada, desfecho, iniciada_em, ultima_atividade_em, turnos, transcricao}]}` — `transcricao` é o diálogo em texto corrido (`[dd/mm hh:mm] Quem: fala`, com as ferramentas usadas em cada resposta), pensado pra uma LLM ler dezenas de conversas em bloco sem ter que remontar o diálogo. `dias` padrão 7 (recusa 0 ou negativo), `limite` padrão 50 e teto 200; `total` continua sendo a contagem real mesmo quando o limite corta — spec 021 |
| `purgar_conversas` | conversas | `conversas:purgar_conversas` | `{}` | `{apagadas, retencao_dias, purga_desativada}` — apaga conversas (e turnos, em cascata) com `ultima_atividade_em` mais antiga que `ConfiguracaoSite.conversas_retencao_dias` (padrão **15**, editável no Admin sem redeploy; **`0` = nunca apagar**, e aí devolve `purga_desativada: true` sem apagar nada). Chamada uma vez por dia pelo workflow do Radar, no fim — depois do envio do resumo, pra falha na purga nunca comer o relatório — spec 021 |
