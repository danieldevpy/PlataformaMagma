...
| `processar_nutridora` | leads | `leads:processar_nutridora` | `{}` | `{processados: [{numero, texto}], total}` — processa a régua T+1/T+3/T+7 dos leads elegíveis (exclui `whatsapp` vazio, `utm_source=whatsapp` e números em `ContatoEscalado`), marca `Lead.nutridora_ultimo_toque` e devolve o texto pronto (dado real: habilidades do curso no T+1, avaliação aprovada no T+3 — se não houver nenhuma ainda, não avança e tenta de novo no dia seguinte —, vagas restantes no T+7 se `exibir_vagas`). No máximo 1 toque por lead por execução, mesmo que o lead esteja atrasado em vários estágios. Chamada por `HTTP Request` comum (sem AI Agent) — spec 020 |
